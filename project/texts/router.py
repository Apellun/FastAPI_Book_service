...
@texts_router.get('/', tags=["texts"], status_code=200, response_model=List[TextRead])  #TODO limit+offset
...
@texts_router.get('/my_texts', tags=["texts"], status_code=200, response_model=List[TextRead])
async def get(user = Depends(user), db: AsyncSession = Depends(get_db)):
    result = await texts_service.get_all_by_author(user.id, db)
    return result

@texts_router.get('/{text_id}', tags=["texts"], status_code=200, response_model=TextRead)
...
@texts_router.post('/', tags=["texts"], status_code=201, response_model=TextRead)
...
@texts_router.put('/{text_id}', tags=["texts"], status_code=200, response_model=TextRead)
...
@texts_router.patch('/{text_id}', tags=["texts"], status_code=200, response_model=TextRead)
...
@texts_router.delete('/{text_id}', tags=["texts"], status_code=204)
