...
@comments_router.get('/{text_id}', status_code=200, response_model=List[CommentRead]) #TODO: just display with texts
async def get_all(text_id: int = None, db: AsyncSession = Depends(get_db)):
    result = await comments_service.get_all(text_id, db)
    return result

@comments_router.post('/{text_id}', status_code=201, response_model=CommentRead)
...
@comments_router.put('/{comment_id}', status_code=200, response_model=CommentRead)
