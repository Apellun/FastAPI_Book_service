...
@books_router.get('/', tags=["books"], status_code=200, response_model=List[BookRead])  #TODO limit+offset
...
@books_router.get('/my_books', tags=["books"], status_code=200, response_model=List[BookRead]) #TODO: doesn't work
async def get(user = Depends(user), db: AsyncSession = Depends(get_db)):
    result = await books_service.get_all_by_author(user.id, db)
    return result

@books_router.get('/{book_id}', tags=["books"], status_code=200, response_model=BookRead)
...
@books_router.post('/', tags=["books"], status_code=201, response_model=BookRead)
...
@books_router.put('/{book_id}', tags=["books"], status_code=200, response_model=BookRead)
...
@books_router.patch('/{book_id}', tags=["books"], status_code=200, response_model=BookRead)
...
@books_router.delete('/{book_id}', tags=["books"], status_code=204)
